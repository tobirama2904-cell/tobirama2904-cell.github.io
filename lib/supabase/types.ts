...
export interface Post { id: string; author_id: string; text: string; image_url: string | null; likes: number; comments: number; reposts: number; created_at: string; author?: Profile; liked?: boolean; reposted?: boolean; }
...
export interface Message { id: string; convo_id: string; sender_id: string; kind: 'text' | 'image' | 'video' | 'file' | 'voice' | 'system' | 'ai'; text: string; media_url: string | null; reply_to: string | null; disappear_at: string | null; pinned?: boolean; created_at: string; sender?: Profile; reactions?: Record<string, string[]>; }
