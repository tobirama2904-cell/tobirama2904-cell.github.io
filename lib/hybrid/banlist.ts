...
export interface BanList {
  admins: string[]; banned: string[]; roles: Record<string, string>; verified: string[];
  updated?: string;
}
const LOCAL_K = 'legion-ban-local-v1';
let cache: { at: number; list: BanList } | null = null;

interface LocalOv { banned: string[]; roles: Record<string, string>; verified: string[]; admins: string[] }
function localOv(): LocalOv {
  try {
    const raw = localStorage.getItem(LOCAL_K);
    if (raw) return { banned: [], roles: {}, verified: [], admins: [], ...JSON.parse(raw) };
  } catch {}
  return { banned: [], roles: {}, verified: [], admins: [] };
}
...
export async function loadBanlist(): Promise<BanList> {
  ...
  let remote: BanList = { admins: [], banned: [], roles: {}, verified: [] };
  try {
...
export async function applyMod(action: 'ban' | 'unban' | 'role' | 'verify' | 'unverify' | 'claimAdmin', pub: string, value = '') {
  const o = localOv();
...
async function publishRemote(token: string, o: LocalOv): Promise<boolean> {
  ...
  const put = async (path: string, branch: string): Promise<boolean> => {
    try {
      ...
      let base: BanList = { admins: [], banned: [], roles: {}, verified: [] };
