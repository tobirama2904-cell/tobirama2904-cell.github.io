...
export function Messenger() {
  ...
  const THEMES = ['', 'bg-gradient-to-b from-sky-500/10 via-transparent to-blue-600/10', 'bg-gradient-to-b from-violet-500/10 via-transparent to-fuchsia-600/10', 'bg-gradient-to-b from-emerald-500/10 via-transparent to-teal-600/10', 'bg-gradient-to-b from-orange-500/10 via-transparent to-rose-600/10', 'bg-gradient-to-b from-zinc-500/10 via-transparent to-zinc-800/20'];
  const THEME_K = 'legion-theme-v1';
  const [themeIdx, setThemeIdx] = useState(0);
  const loadTheme = (cid: string): number => { try { return (JSON.parse(localStorage.getItem(THEME_K) || '{}') as Record<string, number>)[cid] || 0; } catch { return 0; } };
  const cycleTheme = () => {
    if (!active) return;
    const n = (themeIdx + 1) % THEMES.length;
    setThemeIdx(n);
    try { const all = JSON.parse(localStorage.getItem(THEME_K) || '{}'); all[active] = n; localStorage.setItem(THEME_K, JSON.stringify(all)); } catch {}
  };
  ...
  useEffect(() => { setReadDmAt(null); setRoomReads({}); if (active) { const d = loadDraft(active); setInput(d); inputRef.current = d; setThemeIdx(loadTheme(active)); } }, [active]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    <div className={`flex-1 min-w-0 flex-col glass rounded-2xl overflow-hidden ${active ? 'flex' : 'hidden sm:flex'}`}>
      {!activeConvo ? <Empty icon="◈" title="Выбери чат" /> : <>
        <div className="flex items-center gap-2.5 px-4 py-2.5 border-b border-zinc-200 dark:border-white/10">
          ...
          <button title="Тема чата" onClick={cycleTheme} className="p-2 rounded-lg hover:bg-zinc-100 dark:hover:bg-white/10 text-zinc-500">🎨</button>
        ...
        <div ref={anim} className={`min-h-0 flex-1 overflow-y-auto p-4 flex flex-col gap-2 chat-scroll ${THEMES[themeIdx] || ''}`}>
